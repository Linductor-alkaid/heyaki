...
| M10 | [m10-gateway-proxy.md](m10-gateway-proxy.md) | 进行中 | 受限 L4 Gateway 代理：protocol 1.3 变更单、授权/profile、B 侧网关服务、A 侧 API 与可选 SOCKS5 前端。Round 1（2026-09-19）冻结 protocol 1.3 变更单（M10-01/02：bit 13、StreamOpen.gateway、GatewayConnect schema、prelude 与错误映射表、golden vectors、互通回归，CI 35452360300 十一 job 绿）；Round 2（2026-09-19）交付 M10-03/04 与 M10-05 引擎：GatewayProfileConfig+拒绝式校验+NodeConfig 接入、IP/CIDR 原语、内置 deny 表（含 IPv4-mapped 段）、纯准入引擎（profile/端口/CIDR 逐地址/配额 fail-closed），IVA 三轮（抓出并修复 ::1 字节装配等 3 缺陷）55+27 例与全量 35/35 绿。 |
