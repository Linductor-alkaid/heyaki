...
| M10 | [m10-gateway-proxy.md](m10-gateway-proxy.md) | 进行中 | 受限 L4 Gateway 代理：protocol 1.3 变更单、授权/profile、B 侧网关服务、A 侧 API 与可选 SOCKS5 前端。Round 1（2026-09-19）冻结 protocol 1.3 变更单（M10-01/02：bit 13、StreamOpen.gateway、GatewayConnect schema、prelude 与错误映射表、golden vectors、互通回归，CI 35452360300 十一 job 绿）；Round 2（2026-09-19）交付 M10-03/04 与 M10-05 引擎：GatewayProfileConfig+拒绝式校验+NodeConfig 接入、IP/CIDR 原语、内置 deny 表（含 IPv4-mapped 段）、纯准入引擎（profile/端口/CIDR 逐地址/配额 fail-closed），IVA 三轮（抓出并修复 ::1 字节装配等 3 缺陷）55+27 例与全量 35/35 绿。Round 3（2026-09-20）交付 M10-07/08：B 侧 GatewayService（node strand+私有 io strand、准入接线、逐地址拨号+deadline、prelude、有界双泵、idle/duration/配额清扫、会话关闭回收）与 A 侧 Node::open_gateway_stream 公共 API（prelude 内部消费、gateway.use 门、dial deadline）；IVA 两轮抓出并修复 7 生产缺陷（含 SEGV/UAF/泄漏）与 M5 期公共流 API 跨线程竞争（facade 编组至 owning strand，TSan 0 race）；28+55+27+27 例与 36/36 回归绿、asan 0 发现。Round 4（2026-09-20）交付 M10-09/06 与 M10-08 显式化：发起侧 opening 语义+on_connected 一次性回调、B 侧确认流（never/first_use/always、30s fail-closed、TUI yes/no）、SOCKS5 前端组件库 heyaki_socks（loopback-only、CONNECT-only、域名透传、executor 通用池发起 open 防单 asio 线程自死锁）、TUI gateway 视图+CLI profile 配置；IVA 三轮（修 D1 fail_all 回调、D2 自死锁、P1 UAF、P2 结果丢弃、测试栈生命周期）；round4 20 例（19P+1 环境跳过）+全量 37/37、asan/tsan 0 发现。Round 5（2026-09-20）交付 M10-10/11/12：指标面（20 静态族+per-profile，dial P95 毫秒 gauge，m9_slo_rules allowlist 同步）、PeerPathPolicy.gateway_paths 三态、GatewayAuditRecord 审计环与公共 getter、threat-model 增补+安全评审记录（无 P0/P1，遗留 L1/L2/L3）；修复 gateway 专用通道泄漏（顺序第 16 次 open 失败）；M5 调度基准复跑存证；metrics_audit 20 例+全量 38/38 绿。 |
