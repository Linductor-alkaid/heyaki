...
| M10 | [m10-gateway-proxy.md](m10-gateway-proxy.md) | 进行中 | 受限 L4 Gateway 代理：protocol 1.3 变更单、授权/profile、B 侧网关服务、A 侧 API 与可选 SOCKS5 前端。Round 1（2026-09-19）冻结 protocol 1.3 变更单（M10-01/02：bit 13、StreamOpen.gateway、GatewayConnect schema、prelude 与错误映射表、golden vectors、互通回归 27 例 IVA 验证全绿）。 |
