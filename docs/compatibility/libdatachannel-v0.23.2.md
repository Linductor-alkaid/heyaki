...
| Certificate/TLS | OpenSSL, one process-wide TLS family | libdatachannel defaults `USE_GNUTLS=OFF`, `USE_MBEDTLS=OFF`; local CMake selected OpenSSL 3.0.13 | OpenSSL-backed Linux/Windows CI builds passed; release artifacts freeze before M3 |
...
requirement, before M3 starts. TURN/UDP/TCP/TLS must then be tested against the selected coturn
