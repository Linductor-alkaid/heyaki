...
# M4 exit-condition harness: TUI endpoint selection over the merged LAN/relay
# directory must establish an authenticated session and report the actual
# signaling route and data path. Two TUI instances with independent local
# profiles discover each other without any relay, STUN, or TURN server.

[[ "$(uname -s)" == "Linux" ]] || { printf 'SKIP: Linux pty/network harness only\n'; exit 77; }

tui_bin=${HEYAKI_TUI_BIN:-}
work_dir=${HEYAKI_M4_TUI_HARNESS_DIR:-}
...
work_dir=$(mktemp -d "${work_dir:-/tmp}/heyaki-m4-tui-session.XXXXXX")
chmod 700 "${work_dir}"
...
HEYAKI_TUI_SESSION_LOG="${work_dir}/tui-session.log" \
  python3 "${script_dir}/drive_m4_tui_session.py"
